...
from .policy_promotion import PolicyPromotionGuardrails, run_policy_promotion
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Offline evidence-bound promotion gate for a calibration-selected paper policy. "
            "Validation may veto but never selects a fallback policy."
        )
    )
    ...
    parser.add_argument("--promotion-min-selected-policy-folds", type=int, default=3)
    parser.add_argument("--promotion-min-candidate-supporting-folds", type=int, default=2)
    parser.add_argument("--promotion-min-consensus-fraction", type=float, default=2.0 / 3.0)
    parser.add_argument("--promotion-min-candidate-validation-pass-rate", type=float, default=2.0 / 3.0)
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    decomposition = run_stability_decomposition(
        bundle,
        walk_report,
        decomposition_policy,
    )
    guardrails = PolicyPromotionGuardrails(
        min_selected_policy_folds=args.promotion_min_selected_policy_folds,
        min_candidate_supporting_folds=args.promotion_min_candidate_supporting_folds,
        min_consensus_fraction=args.promotion_min_consensus_fraction,
        min_candidate_validation_pass_rate=args.promotion_min_candidate_validation_pass_rate,
    )
    report = run_policy_promotion(bundle, walk_report, decomposition, guardrails)
    print(
        json.dumps(
            report.to_envelope(),
            sort_keys=True,
