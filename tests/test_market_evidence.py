...
def test_evidence_can_bind_public_quote_provenance():
    a = Node("BINANCE_SPOT", "USDT")
    b = Node("BINANCE_SPOT", "BTC")
    route = [
        Edge(a, b, rate=1 / 80_000),
        Edge(b, a, rate=80_800),
    ]
    ...
    receipt = make_evidence_receipt("live-001", route, result, snapshots=[snapshot])
