...
def test_stable_consensus_is_promoted_and_reproducible():
    ...
    assert verify_policy_promotion_bundle_binding(report, walk, decomposition, bundle)
...
def test_calibration_consensus_tie_fails_closed():
    ...
    guardrails = PolicyPromotionGuardrails(
        min_selected_policy_folds=2,
        min_candidate_supporting_folds=1,
        min_consensus_fraction=0.0,
        min_candidate_validation_pass_rate=0.0,
    )
...
def test_latest_calibration_disagreement_blocks_stale_consensus():
    historical = JointPolicyCandidate(30.0, 20.0)
    latest = JointPolicyCandidate(35.0, 40.0)
    support = (
        _support(historical, (1, 2), (1, 2)),
        _support(latest, (3,), (3,)),
    )
    guardrails = PolicyPromotionGuardrails(
        min_selected_policy_folds=3,
        min_candidate_supporting_folds=2,
    ...
    assert decision.status is PolicyPromotionStatus.LATEST_CALIBRATION_DISAGREES
    assert decision.candidate == historical
    assert decision.candidate != latest
...
def test_validation_veto_never_selects_fallback():
    ...
    support = (
        _support(fallback, (1,), (1,)),
        _support(winner, (2, 3), (2,)),
    )
    guardrails = PolicyPromotionGuardrails(
        min_selected_policy_folds=3,
        min_candidate_supporting_folds=2,
        min_consensus_fraction=0.5,
        min_candidate_validation_pass_rate=0.75,
    )
    decision = _decision_for_support(
        WalkForwardStatus.PASSED_STABILITY,
        DecompositionStatus.STABLE_BASELINE,
...
def test_decision_tamper_is_rejected_even_with_recomputed_sha():
    ...
    envelope["payload"]["decision"]["status"] = (
        PolicyPromotionStatus.CONSENSUS_BELOW_FLOOR.value
    )
...
def test_guardrail_types_fail_closed():
    with pytest.raises(ValueError, match="integer"):
        PolicyPromotionGuardrails(min_selected_policy_folds=True)
    with pytest.raises(ValueError, match="finite"):
        PolicyPromotionGuardrails(min_consensus_fraction=float("inf"))
